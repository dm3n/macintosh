...
For an ordinary `mismatch_proof`, first prove that the linked candidate is in the production data path. Then reproduce the exact baseline against immutable evidence and run a fresh, complete after comparison. Populate `production_proof` with every required field from the global contract. The before count must match the contract, the after count must be zero, skips cannot rise, the denominator cannot shrink, before and after evidence IDs cannot overlap, the observation must follow deployment, and adjacent regressions must be zero. Return completed receipts for every before, after, and deployment evidence ID. A backend or agent deployment receipt uses `github-actions:<run_id>:<deployed_commit>` and must identify a successful configured production workflow. A GitHub Deployment receipt uses `github-deployment:<deployment_id>:<deployed_commit>:Production` and must have a successful production status. Opaque strings do not count. If any invariant is unavailable or fails, return `blocked` or `failed`; never manufacture a zero.
